...
# Create Arial font properties with fallbacks
ARIAL_FONT_PROP = fm.FontProperties(family=['Arial', 'Liberation Sans', 'DejaVu Sans', 'Helvetica'])

def apply_vera_font_to_axes(ax):
    """Apply Arial font to all text elements in axes"""
    # Apply to axis labels
    ax.xaxis.label.set_fontproperties(ARIAL_FONT_PROP)
    ax.yaxis.label.set_fontproperties(ARIAL_FONT_PROP)
    
    # Apply to tick labels
    for label in ax.get_xticklabels() + ax.get_yticklabels():
        label.set_fontproperties(ARIAL_FONT_PROP)
    
    # Apply to title if it exists
    if ax.get_title():
        ax.title.set_fontproperties(ARIAL_FONT_PROP)
    
    # Apply to legend if it exists
    legend = ax.get_legend()
    if legend:
        for text in legend.get_texts():
            text.set_fontproperties(ARIAL_FONT_PROP)
...
class StandardPlots:
    ...
    def histogram(data, bins=30, title=None, xlabel=None, ylabel='Frequency',
        ...
        hist_kwargs = {
            'color': plot_config.get_color('primary'),
            'alpha': 0.7,
            'edgecolor': 'white',
            'linewidth': 0.5
        }
